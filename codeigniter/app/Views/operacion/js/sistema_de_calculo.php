<script>
    ...
    var sistemaDeCalculoInterface = {
        ...
        calcularTotalIntereses: function(){
              //calcular interes total
              let NRO_CUOTAS = parseInt(this.NRO_CUOTAS);
            let INTERES_PORCE = (parseFloat(this.INTERES_PORCE) / 12) / 100; // *backup*
            let intereses = this.DETALLE_CALCULO.map(ar => ar.INTERES).reduce((acum, valor) => acum + valor, 0);
            // ((capital_neto_a_desem / NRO_CUOTAS) * (INTERES_PORCE)) * NRO_CUOTAS;
            ...
            let total_prestamo = this.CAPITAL_DESEMBOLSO + intereses + iva_intereses;
        ...
        calcularMontoInteresCuota: function(capital_neto_a_desem) {
            ...
            let tasa = (parseFloat(this.INTERES_PORCE) / 12) / 100;

            let capital = parseFloat(capital_neto_a_desem == undefined ? this.RESULTADOS.CAPITAL_DESEMBOLSO : capital_neto_a_desem);
            //   let interes = capital * (tasa * ma) * dm / da; // *backup*

            let interes = capital * (tasa); // *backup*
        ...
        generarCuotas() {
            ...
            while (CuotaCounter <= TotalCuotas) {
                ...
                let capital_de_cuota = Math.abs(cuota - interes_);
                //Nuevo saldo capital
                SALDO_CAPITAL_ACUM = SALDO_CAPITAL_ACUM - (capital_de_cuota);
                console.log("saldo capital", SALDO_CAPITAL_ACUM, "-", capital_de_cuota);
                //   SALDO_CAPITAL_ACUM = SALDO_CAPITAL_ACUM - (capital_de_cuota + iva_interes); 
                //parseInt(this.SALDO_CAPITAL) - cuota; //mENOS EL MONTO DE CUOTA


                this.INTERESs.push(interes_);
                this.IVAs.push(iva_interes);
                this.CAPITALs.push(capital_de_cuota);
                this.SALDOS_CAPITALES.push(SALDO_CAPITAL_ACUM < 0 ? 0 : SALDO_CAPITAL_ACUM);
    ...
    sistemaFrancesDerivado = {
        ...
        init: function() {
            ...
            this.calcularTotalIntereses();
        },


        calcularAmortizacionSiguiente( amortizacion_anterior){

           return  amortizacion_anterior* (1+( (  this.INTERES_PORCE /12)/100)) ;
        },
        ...
        calcularMontoCuota: function() {
            ...
            importe_de_la_cuota = Math.round(importe_de_la_cuota * 100) / 100;
