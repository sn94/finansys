...
class Prestamo extends BaseController
{
	...
	public function create()
	{
		...
		if ($this->request->getMethod() === 'post') {
			...
			//$CODIGO_OPERACION =  (new Empresa())->gen_numero_operacion($DatosPRESTAMO['EMPRESA']);
			//$DatosPRESTAMO['NRO_OPE'] =  $CODIGO_OPERACION;

			//Datos del formulario 

			//insert o update
			if (   $DatosPRESTAMO['IDNRO']  !=  "") {
				$model->update($DatosPRESTAMO['IDNRO'],    $DatosPRESTAMO);
				return $this->response->setJSON(array("ok" =>   $DatosPRESTAMO['IDNRO']));
			} else {
			...
			/**Empresa */
			$empre = new Empresa_model();
			$empresas = $empre->list_dropdown();
			$tipo_moras = (new Porcentaje_model())->list_dropdown();
			echo view('prestamo/create', ['montos' => $montos,  'empresas' =>  $empresas, 'tipo_moras' => $tipo_moras]);
		}
	}
	...
	public function aprobar($id_prestamo = "")
	{
		...
		if ($this->request->getMethod() === 'post') {
			...
			if ($prestamo_m->ESTADO == "A") {
				return  $this->response->setJSON(['error' => "ESTA OPERACIÓN YA HA SIDO APROBADA ANTERIORMENTE"]);
				//return redirect()->to(base_url("prestamo/index/index"));
			} else {
				$prestamo_m->aprobar();
				return  $this->response->setJSON(['ok' => "APROBADO"]);
				//return redirect()->to(base_url("prestamo/index/index"));
			}
		} else {
			$prestamo_m = new Prestamo_model();
	...
	private function generar_recibo($id_cobro)
	{
		...
		$datos = [
			'IMPORTE' => $TOTAL_IMPORTE, 'DEUDOR' => $DEUDOR, 'IDCOBRO' => $id_cobro,
